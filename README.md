...
5. Download the quality estimation model weights:
   - Download model weights `models.tar.gz` from the provided source
   - Save them in `source/sagemaker/hub/models.tar.gz`

6. Build the quality estimation image:
```bash
cd source/sagemaker/
source ./build_and_push.sh
```

7. Update the `cdk.json` context with your configuration:
```json
{
  "context": {
