...
class SageMakerStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Upload model weights to S3 as a tar.gz archive
        model_path = os.path.join(os.path.dirname(os.path.dirname(__file__)),'source',"sagemaker",'hub',"models.tar.gz")
        model_asset = s3_assets.Asset(
            self, "ModelAsset",
            path=model_path
        )
        
        # Print the S3 object URL for debugging
        print(f"Model asset S3 URL: {model_asset.s3_object_url}")
        ...
        # Grant access to the CDK assets bucket where the model is stored
        ...
        model = sagemaker.CfnModel(
            ...
            primary_container={
                "image": image_uri,
                "modelDataUrl": model_asset.s3_object_url,
                "environment": {
                    "HF_TOKEN": hf_token
                }
            }
        ...
        model.node.add_dependency(model_asset)
        ...
        CfnOutput(
            self, "ModelDataUrl",
            value=model_asset.s3_object_url,
            description="S3 URL for model data",
            export_name="ModelDataUrl"
        )
