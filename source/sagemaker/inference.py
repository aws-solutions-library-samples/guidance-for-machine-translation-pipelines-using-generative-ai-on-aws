...
def get_env_config():
    ...
    logger.info(f"Configuration: USE_GPU={use_gpu}, BATCH_SIZE={batch_size}")
    return {
        'use_gpu': use_gpu,
...
def load_model():
    ...
    try:
        if model is None:
            ...
            # SageMaker places model artifacts at /opt/ml/model when using model_data_url
            sagemaker_model_path = "/opt/ml/model/hub"
            
            if os.listdir(sagemaker_model_path):
                logger.info(f"Using SageMaker model path: {sagemaker_model_path}")
                model_path = download_model("Unbabel/wmt22-cometkiwi-da",saving_directory=sagemaker_model_path,local_files_only=True)
            else:
                model_path = download_model("Unbabel/wmt22-cometkiwi-da", local_files_only=True)
